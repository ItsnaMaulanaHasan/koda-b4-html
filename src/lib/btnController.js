...
import {
  ...
  generateListTasks,
  renameTask,
} from "./tasksController.js"; // Asumsi path file relatif
  ...
  $(() => {
    ...
    $("#btn-dropdown-profile").on("click", function () {
      $(this)
        ...
        .toggleClass(
          "rotate-180 bg-[var(--primary)] bg-[var(--icon-second-color)]"
        );
    });
    ...
    $("#btn-sorting-task").on("click", function () {
      ...
      $(this).toggleClass(
        "text-[var(--primary)] border-[var(--primary)] border-[var(--icon-second-color)] text-[var(--text-secondary)]"
      );
      $(this)
        .children(".icon-dropdown")
        .toggleClass(
    ...
    $(document).on("change", 'input[name="sort-type"]', function () {
      ...
      generateListTasks(currentSortType);
      ...
      $("#btn-sorting-task").html(
        `${btnText}<i class="custom-icon icon-dropdown w-5 h-5 transition duration-300 ease-out bg-[var(--primary)]"></i>`
      );
    ...
    $(document).on("click", ".btn-rename", function (e) {
      e.stopPropagation();
      const idTask = $(this).data("idTask");
      const $taskNameElement = $(this)
        .closest("[id^='option-action-']")
        .siblings(".font-\\[500\\].text-\\[1\\.125rem\\]");
    ...
    $(document).on("click", ".checkbox-subtask", function () {
      const idSubtask = $(this).data("idSubtask");
      $(`#checkbox-${idSubtask}`).toggleClass(
        "bg-[var(--primary)] bg-[url('/assets/img/check.png')] bg-no-repeat bg-center"
      );
